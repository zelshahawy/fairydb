...
pub struct TupleConverter1 {
    _table_schema: TableSchema,
}

impl TupleConverterTrait for TupleConverter1 {
...
mod tests {
    ...
    fn test_conv1() {
        ...
        let tuple_conv0 = TupleConverter1::new(schema.clone());
    ...
    fn test_var_conv1() {
        ...
        let mut buf = [0u8; BUF_SIZE];
        let tuple_conv1 = TupleConverter1::new(schema.clone());
        for t in tuples.iter() {
            let len = tuple_conv1.write_tuple(t, &mut buf, 0).unwrap();
            let tuple2 = tuple_conv1.read_tuple(&buf, 0, len).unwrap();
            assert_eq!(t, &tuple2);
        }
        let mut offset = 0;
        for t in tuples.iter() {
            let len = tuple_conv1.write_tuple(t, &mut buf, offset).unwrap();
            let tuple2 = tuple_conv1.read_tuple(&buf, offset, len).unwrap();
            assert_eq!(t, &tuple2);
            offset += len;
        }
    }
